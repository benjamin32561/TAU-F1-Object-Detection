...
def patch_map(nonground):
    ...
    for pixel in nonground:
        patch = [tuple(pixel[-3:])] # 3: ring ; 4: sector ; 5: zone
        if patch in unique_patches:
            unique_patches[patch].append(pixel)
        else:
            unique_patches[patch] = [pixel]
    return unique_patches

def object_map(hash, threshold):
    for patch in hash.keys():
        if len(hash[patch]) < threshold:
            del hash[patch] # keep only objects
    return hash
...
def split_lidar_record_to_frames(record_path,num_of_frames, format, output_path,pcd_cropper, frame_queue, normalize=False):
    ...
    for i in range(number_of_frames):
        ...
        if frame.success:
            ...
            print("centers shape:" , centers.shape[0])
...
def visulaize_frames(temp_frame_queue):
    ...
    frame_params.ground = frame_params.ground[:, :3]
    frame_params.nonground = frame_params.nonground[:, :3]
    frame_params.centers = frame_params.centers[:, :3]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--record_path', type=str, default='/home/idola/PycharmProjects/TAU-F1-Object-Detection/Lidar/processing/Recordings/', help='path to the lidar record')
