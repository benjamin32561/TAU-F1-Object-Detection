...
struct PointXYZIRingSectorZone
{
  PCL_ADD_POINT4D;                  // Adds the x, y, z fields with SSE alignment
  float intensity;                  // Laser intensity reading
  int ring;                         // Additional field for Ring
  int sector;                        // Additional field for Sector
  int zone;                         // Additional field for Zone

  // Default constructor initializing ring, sector, and zone
  PointXYZIRingSectorZone()
      : intensity(0),  // Default initialization for intensity
        ring(-1),      // Default initialization for ring
        sector(-1),    // Default initialization for sector
        zone(-1)       // Default initialization for zone
  {
      x = y = z = 0.0f;  // Ensure even x, y, z are initialized if not explicitly set
  }
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW   // Ensures proper alignment for new allocations
} EIGEN_ALIGN16;

POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZIRingSectorZone,
                                  (float, x, x)
                                  (float, y, y)
                                  (float, z, z)
...
void PatchWorkpp<PointT>::estimate_ground(
    ...
    if(visualize_)
    {
        cout << "Publishing visualization markers" << endl;
